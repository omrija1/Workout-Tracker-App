...
from ..lib.quote_manager import QuoteManager
...
logging.basicConfig(filename='database_initialization.log',
                    level=logging.DEBUG)
...
class DatabaseManager:
    def __init__(self, database: str):
        self.db_connection_string = os.environ.get(
            'DB_CONNECTION_STRING', f'C:\\Workout-Tracker-App\\{database}')
        ...
        try:
            ...
            with sqlite3.connect(self.db_connection_string) as conn:
                ...
                quote_manager = QuoteManager(
                    self.db_connection_string)  # Initialize QuoteManager
                quote_manager.initialize_quotes_database(
                    conn)  # Initialize quotes database
    ...
    def add_user(self, username: str, email: str, password: str) -> dict:
        ...
        try:
            with sqlite3.connect(self.db_connection_string) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'SELECT 1 FROM profiles WHERE username = ? OR email = ? LIMIT 1', (username, email))
